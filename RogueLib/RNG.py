...
class RNG():
	...
	def set_seed(self, *arguments):
		...
		for j in range(256):
			self.s.append(self.__mash(" "))
			self.s[j] -= self.__mash(" ") * 4.76837158203125e-7
			if self.s[j] < 0:
				self.s[j] += 1
		...
		for i in range(len(args)):
			...
			for j in range(256):
				self.s[j] -= self.__mash(args[i]);
				self.s[j] -= self.__mash(args[i]) * 4.76837158203125e-7
				if self.s[j] < 0:
					self.s[j] += 1
